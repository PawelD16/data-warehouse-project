...
def main(
    ...
    remove_commas(bike_df).to_csv(
        bike_output, index=False
    )
    remove_commas(taxi_df).to_csv(
        taxi_csv_path, index=False
    )


def read_argv_with_defaults(argv: List[str]) -> Tuple[str, str, List[str]]:
    if len(argv) < 4:
        return add_suffix_to_filename(DEFAULT_TAXI_CSV_PATH, DEFAULT_CLEANED_SUFFIX), add_suffix_to_filename(DEFAULT_CSV_OUTPUT, DEFAULT_CLEANED_SUFFIX), DEFAULT_BIKE_CSV_PATHS
